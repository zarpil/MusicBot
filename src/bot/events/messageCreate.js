    ...
    async execute(message, client) {
        ...
        try {
            ...
            if (res.loadType === 'empty' || res.loadType === 'error') {
                const tempMsg = await message.channel.send(t(message.guildId, 'errors.noResultsSimple', { query }));
            ...
            const tempMsg = await message.channel.send(t(message.guildId, 'errors.errorMsg', { message: err.message }));
            setTimeout(() => tempMsg.delete().catch(() => {}), 5000);
        }
    },
};
